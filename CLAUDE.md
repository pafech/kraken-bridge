...
- **Kotlin block comments nest.** Writing MIME type pairs like
  `image/*` or `video/*` inside a `/** ... */` doc comment opens a
  nested block comment at the `/*` and silently swallows the rest of
  the file until a matching `*/`. The compiler complains downstream
  with "Unresolved reference" on instance methods (because half the
  class is now inside a comment), not where the actual nesting started.
  Write MIME pairs in prose ("image vs video") inside doc comments.
...
6. **Vendor compatibility — accessibility layer.** Intent-launch layer
   ...
   `MediaStore.INTENT_ACTION_STILL_IMAGE_CAMERA` with no `setPackage`,
   so the user's chosen default camera opens on every BLE press.
   `openPhotosApp()` resolves the default viewer for the latest media's
   MIME type and passes it to the adapter, which `setPackage`s the
   launch — this avoids the system chooser dialog underwater when no
   system default is set or the default image viewer can't handle video.
   The accessibility-automation layer (shutter tap, photo↔video mode
